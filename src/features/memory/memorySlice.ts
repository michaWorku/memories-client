...
export const createUserMemory = createAsyncThunk(
  ...
  async (formData: Memory) => {
    const {
      data: { memory },
    } = await memoryService.createMemory({ ...formData, user: formData.user })
...
export const getUserMemories = createAsyncThunk(
  ...
  async (options?: {
    ...
    const {
      data: { memories },
    } = await memoryService.getMemories(options?.page, options?.limit, options?.categoryName)
...
export const updateUserMemory = createAsyncThunk(
  ...
  async (formData: CurrentMemory) => {
    const {
      data: { memory },
      // @ts-ignore
    } = await memoryService.updateMemory(formData._id, {
...
export const deleteUserMemory = createAsyncThunk(
  ...
  async (_id: string) => {
    await memoryService.deleteMemory(_id)
  }
)
